...
{
  // allociate memory for b-spline (nx3)-matrix
  ...
  // copy function values f_i into de_boor[1:N+2] (shifted one point)
  for(int i=3; i<N+4; i++){ de_boor[i-2]=values[1][i]; } // and set c_n=f_n
  de_boor[0]=values[1][3];                               // and set c_0=f_0

  // N_{j,4}(x_j)=0, so only the last
  // three entries of the return values of eval_b_splines()
  // are needed
  double temp_N[] = {0,0,0,0};  // { N_{j-3,4},...,N_{j,4} } with N_{j,4}(x_j)=0
  
  // evaluate at x_i and write A = ( N_{j,k}(x_i) )_{i,j<N}
  // rows 1 to (N-1):
  for(int j=1; j<N-1; j++)
    {
